    ...
    public static void Start() {
        ...
        while (true) {
            ...
            if (/*CheckPesel.isPeselGood(sp[2])*/true) {
                ...
                listOfPeople.put(city[i], x[i]);
                for(int j = 0; j<i;j++)
                {
                    if(x[j].getPesel().contains(x[i].getPesel()))
                    {
                        listOfPeople.remove(city[j],x[j]);
                        listOfPeople.add(city[j],x[i]);
                    }
